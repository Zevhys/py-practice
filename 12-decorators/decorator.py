...
"""
! category: medium
TODO: Create a decorator named validate_input that will validate the input of a function before executing it. This decorator must: 1. Check whether all function arguments are of the correct data type, 2. If the data type is incorrect, display an error message and return None, 3. If the data type is correct, execute the function and return its result. Implement this decorator on two different functions: 1. Function multiply_numbers(a, b) that expects input of two numbers (int or float) and returns the product of a and b, 2. Function greet_user(name, times) that expects string input for name and integer for the number of repetitions, then displays the greeting "Hello, [name]!" times number of times. The validate_input decorator must recognize the expected data type of the function parameters through comments or type annotations. When called with correct arguments, the function will run normally. But if the data type is incorrect, for example multiply_numbers("2", 3), the decorator will display an error message like "Error: Parameter 'a' must be of type int, but was given str".
"""
